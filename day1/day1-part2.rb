...
directions.split(", ").each do |var|
	...
	if carddirections[facing] == "N" 
		(current_y_pos + 1).upto(current_y_pos + var.to_i) do |pos|
			current_y_pos = pos
			if squares_visited.include?("#{current_x_pos}, #{current_y_pos}")
				puts "Answer: #{current_x_pos.abs + current_y_pos.abs}"
				exit		
			end
		squares_visited << "#{current_x_pos}, #{current_y_pos}"
		end
	elsif carddirections[facing] == "S"
		(current_y_pos - 1).downto(current_y_pos - var.to_i) do |pos|
			current_y_pos = pos
			if squares_visited.include?("#{current_x_pos}, #{current_y_pos}")
				puts "Answer: #{current_x_pos.abs + current_y_pos.abs}"
				exit		
		...
		(current_x_pos + 1).upto(current_x_pos + var.to_i) do |pos|
			current_x_pos = pos
			if squares_visited.include?("#{current_x_pos}, #{current_y_pos}")
				puts "Answer: #{current_x_pos.abs + current_y_pos.abs}"
				exit		
		...
		(current_x_pos - 1).downto(current_x_pos - var.to_i) do |pos|
			current_x_pos = pos
			if squares_visited.include?("#{current_x_pos}, #{current_y_pos}")
				puts "Answer: #{current_x_pos.abs + current_y_pos.abs}"
				exit		
